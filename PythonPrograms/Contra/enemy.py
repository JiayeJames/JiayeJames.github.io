...
class Enemy(Sprite):
	def __init__(self,game_settings,screen):
		...
		self.screen = screen
		self.image = pygame.image.load('D:\Others\PythonPrograms\Contra\Pictures\enemy\EL\\bag1.png')
		#self.image = pygame.image.load('D:\Others\PythonPrograms\Contra\enemy\EL\bz1.png')
	...
	def boom(self):
		...
		for self.j in range(1,3):
			self.names1['player_image%s' %self.j] = pygame.image.load('D:\Others\PythonPrograms\Contra\Pictures\enemy\\boom%s.png' %self.j)
	...
	def update_run_direction(self,direction):#循环图片
		...
		for self.j in range(1,5):
			self.names1['player_image%s' %self.j] = pygame.image.load('D:\Others\PythonPrograms\Contra\Pictures\enemy\\'+direction+'\\bag%s.png' %self.j)
			self.players1.append(self.names1['player_image%s' %self.j])
